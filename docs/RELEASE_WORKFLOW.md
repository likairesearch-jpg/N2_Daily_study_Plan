...
## Unified main manager

Use the root N2 Sync Manager.vbs. Learning Plan owns Profiles and explicit official-default adoption; Course Drafts owns selected-file context/validation/promotion and separate formal/experiment prompt templates; Publish owns approved-course synchronization and explicit experiment withdrawal; Reference owns manual source refresh. Adoption backs up the previous official configuration and checks preview hashes before saving. It never rebinds existing drafts. Main manager routes formal operations from the selected filename and validates filename/week identity. Development task controls are blocked.
