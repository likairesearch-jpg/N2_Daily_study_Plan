...
Context pack is local, bounded below 100KB and Git-ignored; maximum default candidates: 20 vocab, 10 grammar, 8 kanji, 8 examples. As courses grow, optional reference/review candidate batches shrink to keep the serialized pack below 100KB; rules explicitly report this reduction. Complete covered stable IDs and learner states are preserved. Reading support means vocabulary/readings, not generated reading passages. D+1/D+3/D+7 use day offsets and identify future draft-only items as pending. Red/yellow/green are unknown unless explicitly supplied in local reports/learner-state.json: `{"schemaVersion":1,"states":{"<referenceId>":"red"}}`. This is a user-supplied mapping, not automatic import of browser card colors. The pack includes all matched learned IDs, not claims of mastery; map review reports may need inspection if coverage is sparse. Regenerate context after formal course/reference changes. Next-Week dates follow the preserved Day001 anchor; date drift is for Work/user planning, not silent renumbering.

Week03 remains uncreated by Codex. Creating its context is not authorization to generate a formal course.

## Plan Lab optional additions
Week01-03 bytes remain unchanged. Optional reading.sentences[] uses the existing Japanese text contract (display, tts, lang, translationZh). Without it, rendering splits existing reading text into sentence-sized playback targets. vocabulary.examples[] uses the same contract. For future official Week04+ and experiments this list must be nonempty; legacy collocations are retained, not rewritten into invented example sentences. Future weeks require immutable official profileBinding (see PLAN_PROFILE_SCHEMA.md). Experiment sessions are separate and never participate in formal validation continuity or coverage.
