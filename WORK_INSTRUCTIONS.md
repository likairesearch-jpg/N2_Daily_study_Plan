...
4. 将正式课程写入：
   - `data/course/weekXX.json`
   - 必要时更新 `data/course/index.json`
...
```json
{
  "studyText": "状況（じょうきょう）を把握（はあく）する",
  "tts": "状況を把握する"
}
```
...
截至 2026-09-15，用户已确认 Week 02 / Day 014 之后 App 没有继续更新。

因此：

### 如果 Week 03 不存在、为空、只有草稿或没有正式写入：

从 **Week 03 / Day 015（2026-09-10）** 开始恢复正式更新。
...
Day 021 是 Week 03 第 7 天，必须包含周复盘与交接摘要。
...
正式 Day 以原连续日期记录为准，不因为 App 延迟更新而重新编号。
