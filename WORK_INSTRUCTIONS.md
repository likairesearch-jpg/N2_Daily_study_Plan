...
## Plan Lab interface — latest
Read docs/PLAN_PROFILE_SCHEMA.md, the selected config/profiles/<id>.json and reports/work-context.json. Require matching profileId/hash/snapshot; regenerate context if selecting a different profile. Explicit Profile quantities and weights override ordinary workload defaults. Never invent red/yellow/green, count known items as new, remove D+1/D+3/D+7, or discard useful reading/listening. N2 remains the exam priority; report excessive workload. Official future Week04+ requires profileBinding and vocabulary examples[]. Experiments use an experiment-mode binding and session IDs, not formal Week/Day numbering; write only data/drafts/experiments/<id>.json. Copy/adaptation still requires provenance. Do not rewrite historical Week01-03. No automatic reference refresh.
