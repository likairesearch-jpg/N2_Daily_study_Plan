...
window.N2BuildCards = function(day, h) {
 ...
 for(const module of day.modules) {
  ...
  if(module.type==='review') {
   for(const interval of day.review.intervals){
    ...
    for(const id of interval.vocabularyIds){const v=index.vocabulary.find(x=>x.id===id);if(!v)continue;
     const key=prefix+'-'+id;
     add(key,label,v.word,()=>'<p class="eyebrow">D+'+interval.offsetDays+' · 3秒提取</p><h1>'+ (show(key)?text(v.text):esc(v.word.replace(/（[ぁ-ゖァ-ヺー\s]+）|\([ぁ-ゖァ-ヺー\s]+\)/g,'').trim()))+'</h1>'+ (show(key)?'<p class="meaning">'+esc(v.reading)+' · '+esc(v.meaningZh)+'</p>'+(window.N2PlanClient?.partOfSpeech(id,v.word)||''):reveal(key,'显示读音与意思')));
    }
  ...
  if(module.type==='vocabulary'){
   for(const v of day.vocabulary)add(v.id,label,v.word.display,()=>'<p class="eyebrow">'+(v.mode==='new'?'今日新词':'复习词')+'</p><h1>'+text(v.word)+(window.N2PlanClient?.badge(v.id,v.word.display)||'')+'</h1><p class="translation" lang="ja">'+jp(v.reading,v.reading)+'</p>'+(window.N2PlanClient?.partOfSpeech(v.id,v.word.display)||'')+'<p class="meaning">'+esc(v.word.translationZh)+'</p>'+(window.N2PlanClient?window.N2PlanClient.examples(v.id,v.examples||v.collocations.map(c=>c.text)):examples(v.examples||v.collocations.map(c=>c.text))));
