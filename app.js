'use strict';
const D=window.N2_DAY; let KEY='n2-daily-v1';window.N2ProgressKey=()=>KEY;
...
function addExperiments(){for(const d of window.N2PlanClient?.experimentDays()||[])courseDays.set(d.id,d);}
function weekOptions(){const formal=[...new Set([...courseDays.values()].filter(d=>!d.experimentId).map(d=>d.week))].sort((a,b)=>a-b);return formal.map(w=>'<option value="'+w+'">Week '+String(w).padStart(2,'0')+'</option>').join('')+'<optgroup label="🧪 实验版本">'+((window.N2_EXPERIMENTS||[]).map(e=>'<option value="exp:'+esc(e.id)+'">🧪 '+esc(e.title)+'</option>').join('')||'<option disabled value="experiments-empty">暂无已发布实验课程</option>')+'</optgroup>';}
addExperiments();
function selectDay(id,remember=true){
 if(!courseDays.has(id))id='day001';
 const target=courseDays.get(id),nextKey=target.experimentId?'n2-experiment-v1:'+target.experimentId:'n2-daily-v1';
 if(KEY!==nextKey){save();KEY=nextKey;const defaults={index:0,status:{},note:'',kana:true,rate:1,dayId:id,positions:{},notes:{},revealed:{},answers:{}};try{state={...defaults,...JSON.parse(localStorage.getItem(KEY)||'{}')};}catch{state=defaults;}for(const k of ['status','positions','notes','revealed','answers'])if(!state[k]||typeof state[k]!=='object')state[k]={};remember=false;}
 ...
 $('#day').innerHTML=[...courseDays].filter(([key,d])=>d.week===day.week).map(([key,d])=>'<option value="'+key+'">'+(d.experimentId?'Session '+d.day:'Day '+String(d.day).padStart(3,'0'))+' · '+d.date+'</option>').join('');
 $('#day').value=id;
 $('.eyebrow').textContent=day.experimentId?'🧪 '+day.experimentTitle+' · Session '+day.day:'WEEK '+String(day.week).padStart(2,'0')+' / DAY '+String(day.day).padStart(3,'0');
 document.title=day.experimentId?'N2 · 🧪 '+day.experimentTitle:'N2 每日冲刺 · Day '+String(day.day).padStart(3,'0');
 render();
}
