...
function draft(week,promote=false){
 ...
 try{if(R.signature(R.capture())!==R.signature(before)||fs.readFileSync(draftPath,'utf8')!==text)throw Error('Course/draft changed during validation; retry');
  ...
  R.writeChanged(ROOT,{[target]:text,...(next['data/client-dictionary.json']?{'data/client-dictionary.json':next['data/client-dictionary.json']}: {})});R.writeChanged(ROOT,{'data/publication.json':next['data/publication.json']});R.writeChanged(ROOT,meta);R.bundle(ROOT,next);
...
function context(week,{persist=true}={}){
 ...
 // Keep complete coverage/state metadata; reduce optional candidate batches as courses grow.
 const packBytes=()=>Buffer.byteLength(JSON.stringify(pack,null,2)+'\n');
 if(packBytes()>=100000){
  pack.rules.push('Candidate batches were reduced to fit the 100KB limit; query local reference for additional candidates. Covered IDs and learner states remain complete.');
  const batches=[...pack.spacedReview.flatMap(d=>d.intervals.map(i=>({items:i.candidates,min:4}))),...Object.values(pack.candidates).map(items=>({items,min:2})),{items:pack.reviewRequired,min:2},{items:pack.referenceConflicts,min:2}];
  while(packBytes()>=100000){const available=batches.filter(b=>b.items.length>b.min).sort((a,b)=>Buffer.byteLength(JSON.stringify(b.items,null,2))-Buffer.byteLength(JSON.stringify(a.items,null,2)));if(!available.length)throw Error('Context exceeds 100KB bound; complete coverage metadata requires a smaller context representation');available[0].items.pop();}
 }
if(!persist)return pack;write('reports/work-context.json',pack);write('reports/course-reference-map.json',m);write('reports/reference-mapping-review.json',{courseSignature:m.courseSignature,entries:m.entries.filter(e=>!e.referenceId)});return {file:'reports/work-context.json',bytes:Buffer.byteLength(JSON.stringify(pack,null,2)+'\n'),next:pack.next,coverage:m.summary};
}
module.exports={mapping,draft,context,key};
if(require.main===module)try{const cmd=process.argv[2],week=Number(process.argv.find(a=>a.startsWith('--week='))?.slice(7));const result=cmd==='context'?require('./plan.cjs').context(JSON.parse(fs.readFileSync(path.join(ROOT,'config/active-profile.json'),'utf8')).id,week):cmd==='map'?saveMapping():cmd==='validate'?draft(week):cmd==='promote'?draft(week,true):null;if(!result)throw Error('Use context|validate|promote --week=N, or map');console.log(JSON.stringify(result,null,2));}catch(e){console.error('WORKFLOW FAILED: '+e.message);process.exitCode=1;}
