...
const INPUT=/^data\/(week[0-9]{2,}\.json|day001\.json|knowledge-index\.json|course-policy\.json|publication\.json|client-dictionary\.json|experiments\/experiment-[a-z0-9-]+\.json)$/;
...
function capture(root=ROOT){return Object.fromEntries([...fs.readdirSync(path.join(root,'data')).map(n=>'data/'+n),...(fs.existsSync(path.join(root,'data/experiments'))?fs.readdirSync(path.join(root,'data/experiments')).map(n=>'data/experiments/'+n):[])].filter(n=>INPUT.test(n)).sort().map(n=>[n,read(path.join(root,n))]));}
...
function validate(files){
 ...
 for(const [name,w] of weeks){
  if(!check(w))throw Error(name+': '+ajv.errorsText(check.errors,{separator:'\n'}));if(w.week>3||w.profileBinding)require('./plan.cjs').validateBinding(w.profileBinding,'official');if(w.week>3&&w.days.some(d=>d.vocabulary.some(v=>!v.examples?.length)))throw Error('Future formal vocabulary requires examples[]');
 ...
 let refs;const reference=id=>{if(!refs)refs=new Map(require('./reference.cjs').load().rows.map(r=>[r.id,r]));return refs.get(id);};
 for(const [,w]of weeks)for(const d of w.days)for(const kind of ['vocabulary','grammar'])for(const item of d[kind]||[]){if(item.referenceId){const ref=reference(item.referenceId);if(!ref||ref.type!==(kind==='vocabulary'?'vocab':'grammar')||unit(ref.surface)!==unit((item.word||item.form).display))throw Error(d.id+': invalid referenceId/type/surface '+item.referenceId);}}
 const walk=(x,p)=>{if(!x||typeof x!=='object')return;if(x.provenance){const v=x.provenance;if(!['work-original','third-party-verbatim','third-party-adapted'].includes(v.origin))throw Error(p+': invalid provenance origin');if(v.origin!=='work-original'&&['sourceRef','license','attribution'].some(k=>typeof v[k]!=='string'||!v[k].trim()))throw Error(p+': third-party provenance requires sourceRef, license, attribution');}
 if('display'in x){if(!ajv.validate({$ref:'#/$defs/ja',$defs:check.schema.$defs},x))throw Error(p+': invalid Japanese text (display / tts / lang / translationZh)');}
 ...
 const progressIds=new Set(legacy.cards.map(c=>c.id));const questionIds=new Set();
 const ctx={window:{N2_KNOWLEDGE:index}};vm.runInNewContext(read(path.join(ROOT,'weekly.js')),ctx);
 for(const [,w] of weeks)for(const d of w.days){const state={revealed:new Proxy({},{get:()=>true}),answers:new Proxy({},{get:()=>true,getOwnPropertyDescriptor:()=>({configurable:true,enumerable:true,value:true})})};let cards;try{cards=ctx.window.N2BuildCards(d,{esc:String,jp:String,ruby:String,state});for(const c of cards)c.html();}catch(e){throw Error(d.id+': renderer contract: '+e.message);}for(const c of cards){if(!c.id.startsWith(d.id+'-')||progressIds.has(c.id))throw Error(d.id+': duplicate card ID or incorrect Day prefix: '+c.id);progressIds.add(c.id);}const visit=x=>{if(!x||typeof x!=='object')return;if(x.type==='single_choice'||x.type==='open'){if(!x.id.startsWith(d.id+'-')||questionIds.has(x.id))throw Error(d.id+': duplicate question ID or incorrect Day prefix');questionIds.add(x.id);}for(const [key,v]of Object.entries(x)){if(key==='question'&&Array.isArray(x.questions))continue;visit(v);}};visit(d);}
 return {values,weeks:weeks.map(x=>x[1]),knowledge:index};
}
function metadata(files){const {weeks}=validate(files);const entries=Object.entries(files).filter(([p])=>p!=='data/publication.json').sort().map(([url,text])=>({url,sha256:hash(text)}));const index={schemaVersion:1,experiments:Object.keys(files).filter(p=>p.startsWith('data/experiments/')).map(url=>({url,id:JSON.parse(files[url]).id,title:JSON.parse(files[url]).title})),weeks:weeks.map(w=>({week:w.week,url:'data/week'+String(w.week).padStart(2,'0')+'.json',days:w.days.map(d=>d.id)})),files:entries};const indexText=JSON.stringify(index,null,2)+'\n';const version={schemaVersion:1,courseVersion:hash(indexText),index:'data/index.json',indexSha256:hash(indexText)};return {'data/index.json':indexText,'version.json':JSON.stringify(version,null,2)+'\n'};}
...
function bundle(root,files){const {values,weeks,knowledge:index}=validate(files);writeChanged(root,{'data/day001.js':'// Generated from JSON.\nwindow.N2_DAY='+JSON.stringify(values['data/day001.json'])+';\n','data/course-bundle.js':'// Generated from JSON.\nwindow.N2_DICTIONARY='+JSON.stringify(values['data/client-dictionary.json']||{entries:[],byItem:{},examplesByItem:{}})+';\nwindow.N2_EXPERIMENTS='+JSON.stringify(Object.entries(values).filter(([p])=>p.startsWith('data/experiments/')).map(([,e])=>e))+';\nwindow.N2_WEEKS='+JSON.stringify(weeks)+';\n'+weeks.map(w=>'window.N2_WEEK'+String(w.week).padStart(2,'0')+'=window.N2_WEEKS.find(w=>w.week==='+w.week+');').join('\n')+'\nwindow.N2_KNOWLEDGE='+JSON.stringify(index)+';\n'});}
function assertPublished(files){const manifest=parse(files['data/publication.json']||'{}','publication');if(manifest.schemaVersion!==1||!manifest.weeks)throw Error('Missing publication approvals');const names=Object.keys(files).filter(p=>/^data\/week\d+\.json$/.test(p));if(Object.keys(manifest.weeks).some(p=>!names.includes(p)))throw Error('Publication references a missing week');for(const p of names)if(manifest.weeks[p]?.sha256!==hash(files[p]))throw Error(p+': not promoted or changed after promotion; use draft promote');for(const [p,t]of Object.entries(files))if(p.startsWith('data/experiments/')&&manifest.experiments?.[p]?.sha256!==hash(t))throw Error('Experiment not promoted: '+p);}
function run(command){let files=capture();assertPublished(files);for(const [p,s]of Object.entries(files))if(p.startsWith('data/experiments/'))require('./experiments.cjs').validate(JSON.parse(s));if(command==='prepare'){writeChanged(ROOT,{'data/client-dictionary.json':require('./client-data.cjs').build(files)});files=capture();}const meta=metadata(files);if(command==='validate'){for(const [p,s]of Object.entries(meta))if(!fs.existsSync(path.join(ROOT,p))||read(path.join(ROOT,p))!==s)throw Error(p+': stale metadata; run node tools/release.cjs prepare');}else if(command==='prepare'){writeChanged(ROOT,meta);bundle(ROOT,files);}else if(command==='build'){run('validate');const target=path.join(ROOT,'dist');fs.mkdirSync(target,{recursive:true});for(const p of ['index.html','style.css','app.js','weekly.js','loader.js','plan-client.js','pwa.js','service-worker.js','manifest.json','assets/icon-192.png','assets/icon-512.png']){fs.mkdirSync(path.dirname(path.join(target,p)),{recursive:true});fs.copyFileSync(path.join(ROOT,p),path.join(target,p));}writeChanged(target,{...Object.fromEntries(Object.entries(files).filter(([p])=>p!=='data/publication.json')),...meta});const assets=['service-worker.js','index.html','style.css','app.js','weekly.js','loader.js','plan-client.js','pwa.js','manifest.json','assets/icon-192.png','assets/icon-512.png'];const revision=hash(assets.map(p=>hash(fs.readFileSync(path.join(target,p)))).join(''));const sw=read(path.join(target,'service-worker.js')).replace('__BUILD_REVISION__',revision);fs.writeFileSync(path.join(target,'service-worker.js'),sw);}else throw Error('Use validate, prepare or build');console.log('PASS '+command+': '+validate(files).weeks.length+' weeks; version '+JSON.parse(meta['version.json']).courseVersion.slice(0,12));}
module.exports={ROOT,INPUT,AUTO,hash,capture,signature,validate,metadata,writeChanged,bundle,knowledge,assertPublished,run};
