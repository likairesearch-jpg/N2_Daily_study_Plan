...
function formalCoverage(){
 const R=require('./release.cjs'),{weeks}=R.validate(R.capture()),day1=json(path.join(ROOT,'data/day001.json')),days=weeks.flatMap(w=>w.days);
 return {vocab:new Set([...(day1.vocab||[]).map(v=>surfaceKey(v[0])),...days.flatMap(d=>(d.vocabulary||[]).map(v=>surfaceKey(v.word.display.replace(/（[^）]*）/g,''))))]),grammar:new Set([...(day1.grammar||[]).map(g=>surfaceKey(g[0])),...days.flatMap(d=>(d.grammar||[]).map(g=>surfaceKey(g.form.display.replace(/（[^）]*）/g,''))))])};
}
function selectCandidates(rows,args,covered){
 ...
 const coveredValue=r=>covered[r.type]?covered[r.type].has(surfaceKey(r.surface)):null;
 ...
 return {total:matches.length,offset,limit,nextOffset:offset+limit<matches.length?offset+limit:null,coverageMeaning:'Matched formal course surface; not learner mastery. Kanji/examples unknown.',records};
}
