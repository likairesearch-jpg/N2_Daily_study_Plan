...
(() => {
 ...
 document.querySelector('#export-progress').onclick=()=>{const data=JSON.parse(localStorage.getItem(window.N2ProgressKey?.()||'n2-daily-v1')||'{}');const url=URL.createObjectURL(new Blob([JSON.stringify({format:'n2-progress-v1',storageKey:window.N2ProgressKey?.()||'n2-daily-v1',data},null,2)],{type:'application/json'}));const a=document.createElement('a');a.href=url;a.download='n2-progress-'+new Date().toISOString().slice(0,10)+'.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};
 document.querySelector('#import-progress').onchange=async e=>{try{const file=e.target.files[0];if(!file)return;const v=JSON.parse(await file.text());if(v.format!=='n2-progress-v1'||!v.data||typeof v.data!=='object'||Array.isArray(v.data))throw Error('不是 N2 进度备份');if((v.storageKey||'n2-daily-v1')!==(window.N2ProgressKey?.()||'n2-daily-v1'))throw Error('请先切换到备份对应的正式/实验课程');if(!confirm('导入将合并进度；同名记录使用备份中的值。继续？'))return;const old=JSON.parse(localStorage.getItem(window.N2ProgressKey?.()||'n2-daily-v1')||'{}');const merged={...old,...v.data};for(const k of ['status','positions','notes','revealed','answers'])merged[k]={...(old[k]||{}),...(v.data[k]||{})};localStorage.setItem(window.N2ProgressKey?.()||'n2-daily-v1',JSON.stringify(merged));location.reload();}catch(err){status('导入失败：'+err.message);}};
})();
